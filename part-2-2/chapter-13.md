...
![Mapping of Multicast IP Address to IEEE 802 Multicast MAC Address!](http://www.tcpipguide.com/free/diagrams/arpmulticast.png)

### IP 버전 6의 TCP/IP 주소 결정

주변 탐색 프로토콜(Neighbor Discovery) 정의  
IPv6에서 주변이란 로컬 네트워크에 있는 장비를 가리킨다.  
IPv6에서 주소 결정은 동적으로 이뤄지며 IPv6 주소와 하드웨어 주소 쌍을 유지하는 캐시 테이블을 사용한다. 물리 네트워크의 각 장비는 주변 장비의 정보가 저장된 캐시 정보를 유지한다.

> IPv6는 주소 결정을 위해 ARP 대신 새로운 주변 탐색 프로토콜을 이용한다. IPv6 데이터 그램을 보내고자 하는 장비는 다른 장비의 주소를 얻기 위해 주변 정보 요청 메시지를 보낸다. 가능한 경우, 효율성을 높이기 위해 이 요청 메시지는 브로드캐스트가 아닌 특수한 유형의 멀티캐스트 주소로 전송된다.

- 장비 A가 장비 B에게 데이터그램을 전송한다고 가정한다.
- 장비 A는 메시지를 브로드캐스트하지 않고 장비 B의 요청 노드 주소로 멀티캐스트한다.
- 멀티캐스트된 주변 정보 요청을 받은 각 장비는 주소 결정 대상 장비가 자신인지 여부를 반드시 확인한다. (멀티캐스트는 작은 수의 장비에게만 영향을 준다)
- 장비 B는 주변 정보 요청을 받은 뒤 장비 A에게 주변 정보 광고로 응답한다.
